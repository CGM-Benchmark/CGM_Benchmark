...
def main():
    ...
    parser.add_argument("--ref_batch",required=False, help="path to reference batch npz file")
    parser.add_argument("--sample_batch",required=False, help="path to sample batch npz file")
    args = parser.parse_args()

    # In our format we use a single npz file which have both original(Reference) And Generated Batch
    # So ref_batch and sample_batch path are the same 

    args.ref_batch = args.sample_batch
    args.sample_batch = args.sample_batch
    ...
    ref_acts = evaluator.read_activations_ori(args.ref_batch)
    ...
    return sample_stats.frechet_distance(ref_stats)
...
class Evaluator:
    ...
    def read_activations_ori(self, npz_path: str) -> Tuple[np.ndarray, np.ndarray]:
        with open_npz_array(npz_path, "arr_2") as reader:
            return self.compute_activations(reader.read_batches(self.batch_size))
