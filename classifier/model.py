...
class ResNetModel(nn.Module):
    def __init__(self, **kwargs):
        ...
        self.im_size = kwargs.get('im_size', 64)

        if self.dataset == 'c-blender':
            self.input_channel = 3
            self.emb = nn.Embedding(4, 32)
            self.in_features = 32
        elif self.dataset == 'c-mscoco':
            self.input_channel = 3
            self.emb = nn.Embedding(6, 128)
    ...
    def embed(self, y):
        if self.dataset == 'c-blender' or self.dataset=="c-mscoco":
            return self.emb(y)
        else:
            raise NotImplementedError
