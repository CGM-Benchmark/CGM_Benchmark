...
def save_model(
    glide_model: th.nn.Module, checkpoints_dir: str, train_idx: int, epoch: int, state: int,ddp=True
):
    if ddp:
        th.save(
            glide_model.module.state_dict(),
            os.path.join(checkpoints_dir, f"model-{epoch}x{train_idx}_{state}.pt"),
        )
    else:
        th.save(
    ...
    tqdm.write(
        f"Saved checkpoint {train_idx} to {checkpoints_dir}/model-{epoch}x{train_idx}_{state}.pt"
    )
...
def wandb_setup(
    ...
    device: str,
    base_dir: str,
    project_name: str = "CGM-Benchmark",
):
    return wandb.init(
