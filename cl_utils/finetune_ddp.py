...
def cifar_base_train_step(
    ...
    """
    Perform a single training step for cifar_10 data.

        Args:
            model: The model to train.
            diffusion: The diffusion to use.
            batch: A tuple of (reals, labels, masks,task_ids) 
            device: The device to use for getting model outputs and computing loss.
        Returns:
            The loss.
    """
    reals,labels, masks,task_ids = [x.to(device) for x in batch]
...
def run_cl_epoch(
    ...
    if data_name=="cifar10":
        train_step = cifar_base_train_step
    else:
