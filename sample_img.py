...
class create_cifar(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        return im_1,th.tensor(label_1,dtype=th.long)
...
def main(args):
    ...
    if args.data_name == 'c-blender':
        ...
        # Create 10 scenarios with 10 classes each and 5000 samples per class 
        ...
        correct_set = scenario[args.state-1]
        
        img ,lab,_ = correct_set.get_raw_samples()
        
        new_img = img
        new_lab = lab

        print(np.unique(new_lab,return_counts=True))
        print(f"Size of the data after equal split {len(new_lab)}")
        # Select equal samples from each class 
        new_img = np.array(new_img)
        new_lab = np.array(new_lab)
        correct_set = create_cifar(new_img,new_lab)
