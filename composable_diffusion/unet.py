...
class Energy_UNetModel(nn.Module):
    """
    The full UNet model with attention and timestep embedding.
    :param in_channels: channels in the input Tensor.
    :param model_channels: base channel count for the model.
    ...
    def __init__(
        ...
        self.out = nn.Sequential(
            normalization(ch, swish=1.0),
            nn.Identity(),
            conv_nd(dims, input_ch, out_channels, 3, padding=1), # Removed Zero Module Becacuse of Energy based model Training
        )
        self.use_fp16 = use_fp16


    def forward(self, x, timesteps, y=None, masks=None, layer_idx=None,energy_sampler=False,eval=False):
        """
        Apply the model to an input batch.
        :param x: an [N x C x ...] Tensor of inputs.
        :param timesteps: a 1-D batch of timesteps.
